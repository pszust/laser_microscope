...
class Window(Frame):
    ...
    def init_window(self):
        ...
        cam_params_frame = Frame(root)
        cam_params_frame.grid(row=row, column=col, padx = padd)
        row += 1
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_BRIGHTNESS: ')
        self.label.pack(side = LEFT)        
        self.brightness_var = StringVar()
        self.brightness_var.set('5.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.brightness_var)
        self.cam_entry.pack(side = LEFT)         
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_CONTRAST: ')
        self.label.pack(side = LEFT)        
        self.contrast_var = StringVar()
        self.contrast_var.set('5.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.contrast_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_SATURATION: ')
        self.label.pack(side = LEFT)        
        self.sat_var = StringVar()
        self.sat_var.set('5.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.sat_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_GAIN: ')
        self.label.pack(side = LEFT)        
        self.gain_var = StringVar()
        self.gain_var.set('5.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.gain_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_EXPOSURE: ')
        self.label.pack(side = LEFT)        
        self.exp_var = StringVar()
        self.exp_var.set('5.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.exp_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_AUTO_EXPOSURE: ')
        self.label.pack(side = LEFT)        
        self.aexp_var = StringVar()
        self.aexp_var.set('1.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.aexp_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)        
        self.label = Label(temp_frame, text = 'CAP_PROP_AUTO_WB: ')
        self.label.pack(side = LEFT)        
        self.awb_var = StringVar()
        self.awb_var.set('1.0')
        self.cam_entry = Entry(temp_frame, width = 15, textvariable = self.awb_var)
        self.cam_entry.pack(side = LEFT) 
        
        temp_frame = Frame(cam_params_frame)
        temp_frame.pack(fill = X)    
        self.buttonSet = Button(temp_frame, text = 'Set parameters', command = self.set_cam_params)
        self.buttonSet.pack(side = LEFT)     
        self.buttonGet = Button(temp_frame, text = 'Get parameters', command = self.get_cam_params)
    ...
    def get_cam_params(self):
        ...
        for i in range(0, len(camera_variables)):
            camera_variables[i].set(str(cap.get(camera_parameters[i])))
    ...
    def set_cam_params(self):
        ...
        for i in range(0, len(camera_variables)):
            cap.set(camera_parameters[i], float(camera_variables[i].get()))
